...
Pick a value where the quantities differ. When a rule says that one thing determines another, the determining thing is what has to vary: if every test supplies its most ordinary form, the rule is untested no matter how many tests there are, and adding more in the same shape cannot find the defect.

The trap closes when the ordinary form is also the convenient one, because then every author independently reaches for it and the whole suite comes to rest on the single input that cannot distinguish anything. A suite in that state is not weakly tested in places. It is silent about one rule everywhere at once.

## Coverage Counts Visits, Not Variety

A line reported as covered is a line that ran, and nothing more.

Total branch coverage is necessary and it is not sufficient. Coverage asks whether control reached a place, never whether it arrived carrying a value that could tell right from wrong. Code exercised only by inputs that cannot fail is reported as covered and reported as tested, and it is neither.

So a coverage requirement met in full is not evidence that a rule holds. It is evidence that the lines implementing the rule are reachable. When the number is the only thing standing behind a behaviour, the behaviour is unverified.
