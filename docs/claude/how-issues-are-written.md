...
The program is the code a test tier can run. That is the simulator sources under `src/` and the C++ beside them in `lib/`, the Python under `scripts/` and `lib/python/`, and the machinery under `test/` that computes the values assertions rest on — fixtures, helpers, and readings such as `test/lib/python/workflow_gates/`, each of which carries its own `unit/` directory and its own coverage gate. A defect in any of that got past a tier which exists and could have failed. Naming the assertion that let it through is what turns one report into a described gap in the suite.
