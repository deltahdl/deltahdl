...
Prefer positive instructions to prohibitions when writing or editing the prompts this repository's scripts feed to a spawned session. Those are `build_lrm_read_instruction` and the step pipeline in `satisfy_subclause/mutators.py`, and the equivalent in `satisfy_subclauses`.
