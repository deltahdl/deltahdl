...
Bulk-extracting page text through `pypdf` — `page.extract_text()` on any page — blows the same budget, and afterwards even printing the length of the result is suppressed. The suppression then poisons everything else in the turn: `echo`, local file reads, all of it.
