...
Deleting a declaration is right only when the two really do make one claim — in practice when they are letter-suffix siblings covering the same clause, where one is a shallower restatement of the other. Across an annex and a clause file, prefer renaming: the coverage was put there on purpose.
...
A comment above a renamed declaration saying what it covers and which sibling file carries the other case keeps the pair discoverable from either end.
...
Related: [test-file-letter-suffixes](test-file-letter-suffixes.md) for the family a deletion case usually belongs to, and [verifying-through-ci](verifying-through-ci.md) for reading the result.
