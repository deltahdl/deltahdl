...
Keep in the local directory only what is true of this machine and useless elsewhere. The standing example is the retention rule: per-subclause satisfaction state belongs in the GitHub issue and in `git log`, never in a memory file, so retire a memory that turns out to be a per-subclause write-up rather than repairing it. A session spawned by `scripts/satisfy_subclause/mutators.py` writes no memories at all, because `build_env` and `write_deny_hook_settings` in `lib/python/claude_cli_streaming` turn auto-memory off from both directions.
