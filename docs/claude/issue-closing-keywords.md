...
The mirror image of that trap is a comma-separated list. A keyword binds to exactly one number, so `Closes #N, #M, #P` closes the first and leaves the rest open while reading as though it closed all three. A commit that finishes several issues needs the keyword repeated, one per line:
