...
One very large tool result exhausts the same content-filter budget that batched PDF reads do. Reading the roughly 1480-line `src/simulator/vpi.h` in a single call was enough. After it trips, every later tool result in the turn renders as `... [truncated]`, including `echo OK`, and it does not recover within the turn.
