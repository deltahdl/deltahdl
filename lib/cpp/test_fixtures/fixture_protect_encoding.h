...
// about where it ends, so the block runs to the next `pragma directive:
// §34.5.9.2's alphabets spell no directive, which is what makes that line the
// one a block can never carry. A bare backtick is not it: uuencode writes the
// grave accent for a six-bit zero, so a line of one is the terminator its own
// output ends with.
inline std::string EncodingDataBlockLinesOf(std::string_view envelope) {
  ...
  std::string_view rest = envelope.substr(opens + kEncodingBlockOpening.size());
  std::string block;
  while (!rest.empty()) {
