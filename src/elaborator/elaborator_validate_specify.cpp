  ...
  for (auto* item : mod->items) {
    ...
    for (size_t j = 0; j < edge_paths.size(); ++j) {
      for (size_t i = 0; i < j; ++i) {
        if (!SameEndpoints(edge_paths[i]->path, edge_paths[j]->path)) continue;
        if (!ConsistentRefs(edge_paths[i]->path, edge_paths[j]->path)) {
          diag.Error(edge_paths[j]->loc,
      ...
      if (si->kind == SpecifyItemKind::kPulsestyle) {
        for (std::string_view sig : si->signal_list) {
          if (path_dsts.contains(sig)) {
            diag.Error(si->loc, std::format("pulsestyle declaration for '{}' "
