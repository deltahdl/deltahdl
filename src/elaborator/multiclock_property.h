...
// sequence used as a subproperty makes the whole property multiclocked even when
// every explicit clock matches the property clock.
//
// `subproperty_clocks` lists the governing clock of each subproperty in source
// order; `any_subproperty_is_multiclocked_sequence` is set when some subproperty
// is a multiclocked sequence (whose legality is established in §16.13.1).
