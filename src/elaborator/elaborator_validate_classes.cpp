...
// §8.18 (printed page 194 of IEEE 1800-2023): the report for a member
...
// it names, read along the chain §8.23 (printed 200) gives it: the leftmost
// identifier is a package §26.3 (printed 808) resolves the next one through,
// or a class by its bare name, and each identifier after that is a class
// nested in the class so far. `C`, `p::C`, `Outer::Inner` and
// `p::Outer::Inner` each resolve; a typedef or a parameterized class in the
// chain answers empty and leaves the access as silent as it was.
//
// This answered the named class alone, so the enclosing classes were lost
// before ClassOfDeclaredType read the type a member of that class was
// declared with.
static std::vector<const ClassDecl*> ClassChainOfScopePrefix(
...
static const ClassMember* ScopedClassMember(const Expr* e,
  ...
  const std::vector<const ClassDecl*> kChain =
      ClassChainOfScopePrefix(e->lhs, unit);
  return kChain.empty() ? nullptr
                        : FindMemberInClass(kChain.back(), e->rhs->text, unit);
}
...
// The class a property of the last class of `chain` declared with `dt` holds
// a handle to: `C m_inst;` by the bare name, `p::C m_inst;` through the
// package, `Outer::Inner m_inst;` through the enclosing class. A bare name is
// read along the chain outward and then among the unit's classes; a scoped
// name's prefix is read the same way, then as a package, and the type is the
// class nested in whichever answers.
//
// This took the owning class alone and read a bare name as the owner, a class
// nested in it or a class of the unit, and a scoped prefix as a package or a
// class of the unit, so `A` and `Outer::A` written inside B, A being B's
// sibling in Outer, resolved to no class and `Outer::B::a.k` on a local `k`
// was accepted from a module.
static const ClassDecl* ClassOfDeclaredType(
    const DataType& dt, const std::vector<const ClassDecl*>& chain,
    const CompilationUnit* unit) {
...
static const ClassDecl* HandleClassOfBase(
  ...
  const std::vector<const ClassDecl*> kChain =
      ClassChainOfScopePrefix(base->lhs, unit);
  if (kChain.empty() || base->rhs->kind != ExprKind::kIdentifier) {
    return nullptr;
  }
  const ClassMember* m =
      FindMemberInClass(kChain.back(), base->rhs->text, unit);
  if (!m || m->kind != ClassMemberKind::kProperty) return nullptr;
  return ClassOfDeclaredType(m->data_type, kChain, unit);
}
