...
// analog of the set_covergroup_expression that defines a coverpoint bin (see
// the §19.5.1.2 helpers). The cross_set_expression production itself belongs to
...
// permitted as long as it evaluates to the cross's CrossQueueType, and a cast
// is required when the expression's type is assignment-incompatible with it.
// The helpers below carry that compile-time type rule for the elaborator.
...
// assigned. Returns true when the expression's type may be used as written,
// i.e. it is assignment compatible with the cross's CrossQueueType, reusing the
