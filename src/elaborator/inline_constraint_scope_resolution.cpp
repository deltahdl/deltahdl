...
InlineConstraintBinding ResolveInlineConstraintName(
  ...
  switch (qualifier) {
    ...
    case InlineConstraintQualifier::kLocal:
      // §18.7.1: local:: bypasses the object's class scope and resolves the name
      // in the local scope, identically to the unqualified name written there.
    ...
    case InlineConstraintQualifier::kNone:
      // §18.7.1: an unqualified name searches the object's class scope first and
      // falls back to the local scope only when the class does not declare it.
