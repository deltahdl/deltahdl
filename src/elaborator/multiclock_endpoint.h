...
// application of `triggered` appears. (The same rule is stated in §16.9.11.)
...
// applicable method for the given source/destination clocks (i.e., they differ).
...
// to which `matched` is applied. The same restrictions apply as for `triggered`,
// and values of local variables sampled in such an instance flow out under the
// same conditions as for `triggered` (see §16.10).
...
// matches are treated semantically the same way as matching both disjuncts of an
// `or`; the evaluating thread forks to account for the distinct local-variable
// valuations.
bool MatchedMultipleMatchesTreatedAsOrDisjuncts();

// §16.13.5: unlike `triggered`, `matched` stores the result of the source
// sequence match until the arrival of the first destination clock tick after the
// match, synchronizing between the two clocks. Reuses the persistence model from
// §16.13.6.
