...
// argument of an instantiated checker obtains its effective value. A checker may
// declare a formal whose default value is a context value function (see
...
// themselves belong to §16.14.7, and the legality of an instantiation belongs to
// §17.3.
...
// supplied). An omitted optional formal falls back to its declared default; that
// default is taken from the instantiation context when it is a context value
// function (as in my_check2) and used verbatim otherwise.
...
// at the point where the checker is instantiated, not where it is declared, so a
// single checker declaration adapts its inferred clock and disable to each
// instantiation context.
