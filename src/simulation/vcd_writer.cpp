...
void VcdWriter::add_signal(std::string_view scope, std::string_view name,
                           uint32_t width) {
  SignalEntry entry;
...
void VcdWriter::write_signal_defs(const std::string& scope) {
  for (const auto& sig : signals_) {
    ...
    file_ << "$var wire " << sig.width << ' ' << make_id_string(sig.id) << ' '
          << sig.name << " $end\n";
  }
}
...
void VcdWriter::write_scalar_change(const Logic4Vec& value,
