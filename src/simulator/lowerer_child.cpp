...
static void CreateChildModuleNets(const std::string& inst_prefix,
  ...
  for (const auto& net : resolved->nets) {
    ...
    // §6.7.1 with §7.2.1: the instance's net of a packed structure is laid
    // out under its prefixed name, the key a member select of it is resolved
    // by, as RegisterModuleNets lays the top's out.
