...
// §17.7.2 describes assume-based checker variable randomization: how a simulator
// periodically solves the assume statements of a checker instance to find values
// for its free checker variables and then updates them. The helpers here model
// only the rules the text of §17.7.2 states directly — which variables are
// active, how the assume set is formed, what a solution attempt does, when free
// variables are randomized and how long they hold, and where in the region order
// the solve and any resulting failures occur. Each rule is encoded as a pure
// function so it can be observed in isolation. The active/inactive analogy to
// rand variables is owned by §17.9, the sampled-value definition by §16.5.1, the
// region order by §4 scheduling, and the checker-instance lifetime by §17.3;
// this file only records how §17.7.2 scopes those onto checker randomization.
...
// procedural call. Returns true only for the assume set clock event trigger.
bool AssumeRandomizationIsTriggeredBy(RandomizationTrigger trigger);

// §17.7.2: there is no randomize() for checkers, so checkers expose no procedural
...
enum class AssumeRandomizationVariable : uint8_t {
  // A non-const free checker variable that does not appear on the left-hand side
...
// every other variable — a non-free checker variable or a checker formal — is
// always inactive. Returns true when the variable is treated as active.
...
bool AssumeRandomizationElementsAreSeparatelyActive(CheckerVariableAggregate agg);
...
// supplies an explicit initializer. Returns true when randomization supplies the
// initial value, i.e., when the declaration has no initializer.
bool FreeCheckerVariableTakesRandomInitialValue(bool has_declaration_initializer);
...
// from the assume set; among the remaining statements, only those referencing an
// active free checker variable are included. Returns true when the statement is
// a member of the assume set.
...
// §17.7.2: a solution attempt seeks values for all active checker variables such
// that, together with the inactive variables and state, none of the assumptions
// fail in that time step. The attempt is successful exactly when such a set of
// values is found; otherwise any values may be chosen for the active variables
...
// §17.7.2: that implicit assume set clock event of an empty assume set is placed
// before the Observed region. Returns the region in which it occurs.
...
enum class AssumeSetClocking : uint8_t {
  ...
  kExplicitlyClocked,  // active variables of a checker with a nonempty assume set
};

// §17.7.2: active variables in a checker with an empty assume set are implicitly
// clocked; those in a checker with a nonempty assume set are explicitly clocked.
...
// §17.7.2: an active variable that does not appear in any property of a nonempty
// assume set is unconstrained, yet still explicitly clocked. Returns its
// clocking.
AssumeSetClocking ActiveVariableAbsentFromNonemptyAssumeSetClocking();

// §17.7.2: such an active variable is unconstrained. Returns false: it carries no
// constraint.
bool ActiveVariableAbsentFromNonemptyAssumeSetIsConstrained();

// §17.7.2: when an implementation is about to begin the Observed region it shall
// solve for all the active free checker variables. Returns the region whose
// beginning triggers that solve.
Region AssumeRandomizationSolvePrecedesRegion();

// §17.7.2: the solve uses sampled values of all other variables, and the sampled
// value of an active checker variable is its current value (see §16.5.1), rather
// than a preponed value. Returns true: the sampled value is the current value.
