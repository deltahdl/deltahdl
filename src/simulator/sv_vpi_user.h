 ...
 *
 * §M.2 shows the file, and this is it as the annex shows it: the object
 * types, the methods that traverse one-to-one and one-to-many relations, the
 * generic object properties and their return values, the operators, the
 * callback reasons, the coverage VPI and the assertion VPI, each constant
 * carrying the annex's number and standing in the annex's order, then the
 * two assertion structures and the one routine the file declares. The
 * annex's note on the numbers: the values 600 through 999 are reserved for
 * this file, 600 through 749 for the SV VPI model extensions, 750 through
 * 779 for the coverage VPI and 800 through 899 for future use, and a number
 * may be shared across the categories of identifier -- object types,
 * properties and callbacks -- as vpiClockingBlock and vpiQualifier share
 * 650 below, so a constant is read against the category the routine it is
 * passed to expects.
 *
 * The one constant the annex does not list is vpiNetTypedef, the tag §37.10
 * writes on the iteration from an instance to the nettype declarations it
 * holds; the annex names the declaration's own type, vpiNettypeDecl, and no
...
/* §37.10: the iteration from an instance to its nettype declarations, which
 * the diagram tags vpiNetTypedef and Annex M does not list; 711 is a number
 * the annex leaves unused. The objects it reaches are of type vpiNettypeDecl.
 */
#define vpiNetTypedef 711
...
/* callback on class object reclaimed by automatic memory management */
