...
// next, so `pending` counts the instances placed since the last tick and
// every one of them begins an attempt at the next, a statement a loop
// reaches several times in one step beginning as many. The named scopes the
// statement was first reached in are kept for its reports to name, as
// §21.2.1.5's %m names the statement under the labels of the procedure.
...
// process keeps for it, answering false where it keeps none, a process the
// lowerer started no monitor for, so that the caller evaluates the
// statement where it stands.
