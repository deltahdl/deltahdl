...
// and the §16.7 cycle delay before each, the first one the leading delay.
...
// clause has it and an instance of a sequence declared without a clock
// inheriting `seq`'s. The delay before the instance and the instantiated
// body's leading delay add. Answers false where `seq` has no linear body the
// parser captured, where an instance names a sequence that has none, or where
// instances nest past the depth a cyclic dependency, which §16.8 makes an
// error, would reach.
