...
void parse_define(std::string_view def, CliOptions& opts) {
    ...
    opts.defines.emplace_back(std::string(def.substr(0, eq)), std::string(def.substr(eq + 1)));
}

bool try_parse_valued_arg(std::string_view arg, int& i, int argc, char* argv[], CliOptions& opts) {
    if (arg == "--top" && i + 1 < argc) { opts.top_module = argv[++i]; return true; }
    if (arg == "--vcd" && i + 1 < argc) { opts.vcd_file = argv[++i]; return true; }
    if (arg == "-o" && i + 1 < argc) { opts.output_file = argv[++i]; return true; }
    if (arg == "--max-time" && i + 1 < argc) { opts.max_time = std::stoull(argv[++i]); return true; }
    if (arg == "--seed" && i + 1 < argc) { opts.seed = std::stoul(argv[++i]); return true; }
    return false;
}

bool try_parse_flag(std::string_view arg, CliOptions& opts) {
    if (arg == "--version") { opts.show_version = true; return true; }
    if (arg == "--help") { opts.show_help = true; return true; }
    if (arg == "--synth") { opts.synth_mode = true; return true; }
    if (arg == "--lint-only") { opts.lint_only = true; return true; }
    if (arg == "--dump-ast") { opts.dump_ast = true; return true; }
    if (arg == "--dump-ir") { opts.dump_ir = true; return true; }
    if (arg == "-Wall") { opts.wall = true; return true; }
    if (arg == "-Werror") { opts.werror = true; return true; }
    return false;
}
...
bool parse_args(int argc, char* argv[], CliOptions& opts) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (try_parse_flag(arg, opts)) { continue; }
        if (try_parse_valued_arg(arg, i, argc, argv, opts)) { continue; }
        if (arg.starts_with("+define+")) { parse_define(arg.substr(8), opts); continue; }
        if (arg.starts_with("+incdir+")) { opts.include_dirs.emplace_back(arg.substr(8)); continue; }
        if (arg.starts_with("-")) {
...
std::string preprocess_sources(const CliOptions& opts, delta::SourceManager& src_mgr, delta::DiagEngine& diag) {
...
delta::CompilationUnit* parse_source(
    std::string& source, delta::SourceManager& src_mgr, delta::DiagEngine& diag, delta::Arena& arena) {
...
int run_simulation(const CliOptions& opts, delta::CompilationUnit* cu, delta::DiagEngine& diag, delta::Arena& arena) {
    ...
    if (diag.has_errors() || design == nullptr) { return 1; }
...
int main(int argc, char* argv[]) {
    ...
    if (opts.werror) { diag.set_warnings_as_errors(true); }

    auto source = preprocess_sources(opts, src_mgr, diag);
    if (source.empty() || diag.has_errors()) { return 1; }
