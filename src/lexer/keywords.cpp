...
// Keyword data lives in a namespace-scope constexpr table: it is constant-
// initialized (no throwing dynamic init) and is not a function body (so it does
// not count against the function-size threshold). KeywordMap() builds the
// lookup map from it lazily on first use.
constexpr std::pair<std::string_view, KeywordEntry> kKeywordTable[] = {
...
const std::unordered_map<std::string_view, KeywordEntry>& KeywordMap() {
  static const auto kMap = [] {
    std::unordered_map<std::string_view, KeywordEntry> m;
    m.reserve(std::size(kKeywordTable));
    for (const auto& [name, entry] : kKeywordTable) {
      m.emplace(name, entry);
    }
    return m;
  }();
  return kMap;
...
const std::unordered_set<std::string_view>& NoconfigExcluded() {
...
const std::unordered_map<std::string_view, KeywordVersion>& VersionMap() {
  static const std::unordered_map<std::string_view, KeywordVersion> kMap = {
...
std::optional<KeywordVersion> ParseKeywordVersion(std::string_view spec) {
  const auto& version_map = VersionMap();
  auto it = version_map.find(spec);
...
std::optional<TokenKind> LookupKeyword(std::string_view text,
  ...
  const auto& keyword_map = KeywordMap();
  ...
  if (version == V::kVer13642001Noconfig && min_ver == V::kVer13642001 &&
      NoconfigExcluded().contains(text)) {
    return std::nullopt;
  }
  return kind;
}
