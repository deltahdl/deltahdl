...
`start bysubclause` takes the issue `next_subclause` names, and beyond it takes what it finds on the way. Where a defect it turns up while solving that issue stops the issue from closing, it solves that first and returns to the subclause. What it leaves is the rest of the tracker: the open issues the command does not name are somebody else's finding rather than this loop's. Matching is on the canonical `Satisfy IEEE 1800-2023 §<subclause>` title, so those issues are outside this form by construction.
...
```text
REMINDER: Run PYTHONPATH=.:scripts python3 -m next_subclause for the subclause in force and the issue tracking it; solve that issue whatever its number, and when it closes the same command names the next. Where a defect you find while solving that issue stops it from closing, solve that first and then return to the subclause. The open issues the command does not name are not this loop's work.
```
