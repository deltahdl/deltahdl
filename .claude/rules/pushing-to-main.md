...
The habit is worth naming because it comes back. It is not written down anywhere, so nobody can find it and drop it by reading the tree, and it returns whenever a message is composed from ordinary git practice rather than from this file. That is why it is recorded here as a decision rather than left to be re-derived.
