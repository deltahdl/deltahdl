...
The program is the code a test tier can run. That is the simulator sources under `src/` and the C++ beside them in `lib/`, the Python under `scripts/` and `lib/python/`, and the Python and C++ under `test/` that computes the values assertions rest on — the fixtures, the helpers, and the readings of the standard such as `lib/python/lrm/`, each of which carries its own `unit/` directory under `test/` and its own coverage gate. A defect in any of that got past a tier which exists and could have failed. Naming the assertion that let it through is what turns one report into a described gap in the suite.
...
Within a program issue, the three backward-looking sections are the point of the format rather than padding. A defect that reached `main` got past every gate CI runs, and naming the assertion that let it through turns one report into a described gap in the suite. Answer each tier honestly, including when the honest answer is that the tier does not exist for this code. "There is no such tier here" is a finding to write down rather than a section to drop.

Read what a tier observes off the file that decides it, and not off another issue. For the e2e tier that file is `scripts/run_sim_tests/__init__.py`: `visible_output` returns the simulator's standard output concatenated with its standard error, with the repository root cut out of the text, and `run_test` compares that against the `.expected` file. A rejection is expressible there, and `test/src/e2e/end_keywords_unmatched.expected` holds one, its `§22.14` report and caret line included. A pair needs no registration, being discovered by a glob over `test/src/e2e/`, and a `.sv` with no `.expected` is skipped rather than failed. So a section concluding that the tier is closed to a defect has to say why the pair could not be written. "A diagnostic goes to standard error" is not that reason, and #3139, #3140, #3141 and #3160 each gave it before being corrected, writing off a tier that would have caught what they described.
