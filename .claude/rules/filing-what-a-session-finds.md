...
Most findings arrive while working on something else. Finish the work in hand first, then file, so the issue is written from what the reading established rather than from what the fix happened to touch.
...
Related: [lowerbinarybit-default-arm](../memories/lowerbinarybit-default-arm.md) for the reading these rules were drawn from.
