...
**Why:** The user said so repeatedly on 2026-09-18, as a standing reminder, while #3617 on the sv-tests runner was being brought up to date. That suite names its files and tags by IEEE 1800-2017, and its file names — `18.5.10--variable-ordering`, `18.5.14--soft-constraints` — carry numbers one above the 2023 headings of the same title (§18.5.9 on physical page 540, §18.5.13 on 547 of `~/LRM.pdf`). A reader could take the mismatch as a reason to make the tool cite the suite's numbers, or to reason from what the 2017 text says. Neither: the tool's report cites the 2023 subclause whose rule it enforces, whatever translation the suite needs belongs to the runner's scoring alone, and the only facts to reason from are the suite's own file names and the 2023 pages, since no other edition is on hand to be checked.
