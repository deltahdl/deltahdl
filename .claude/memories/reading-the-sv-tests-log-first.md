...
**Why:** `print_reason` in `scripts/run_sv_tests/__init__.py` prints deltahdl's own output under every FAIL line, and its docstring, until the assert-no-comments gate removed every docstring on 2026-09-14, gave the reason: "A line naming the file that failed says nothing about why it failed, so whoever picks the failure up has to run deltahdl over that file themselves to find out -- and working it out from the source instead is a reliable way to reach a confident wrong answer. The output was captured when the test ran; this puts it where the run can be read afterwards."
