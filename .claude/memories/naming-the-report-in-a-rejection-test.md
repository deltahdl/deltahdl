...
**How to apply:** Name the report. Leave two kinds of site alone: one that varies only the construct under test while holding the rest of the source fixed, and one whose rule nothing reports, which is a defect in the program to solve in the session that found it — see [solving-what-a-session-finds](solving-what-a-session-finds.md). A test asserting a source was accepted is sound as it stands.
