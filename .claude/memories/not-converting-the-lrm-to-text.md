...
**How to apply:** Read pages with the Read tool, one per call, per [reading-the-lrm-one-page-per-call](reading-the-lrm-one-page-per-call.md). A Bash deny hook already blocks `pdftotext`, `pdfgrep`, `pdftohtml`, `pdftoppm` and `mutool`; it allows `pdfinfo` and `python3` with `pypdf`, so `pypdf` stays a live hazard rather than a blocked one. Do not reach for it even to read the outline: the user asked on 2026-09-14 that the LRM be read through the Read tool alone — see [locating-a-clause](locating-a-clause.md).
