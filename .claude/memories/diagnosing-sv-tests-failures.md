...
description: How to run one failing sv-tests file locally, which the never-build-locally rule allows as its second exception.
metadata:
  type: project
---
...
Read the CI log before building, because that printing means a file rejected by the simulator is already diagnosed there. What the log drops is the rest of a simulated run's stdout: `check_assertions` keeps the first `:assert:` line that did not hold and discards every other line the run printed. That gap is the case this file is about, and it is the whole of what [verifying-through-ci](../rules/verifying-through-ci.md) allows building for.
