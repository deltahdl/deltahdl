...
**How to apply:** a convention learned in a session still gets a section in `.claude/CLAUDE.md` and a note in `.claude/rules/`. Everything the memory format covers — who the user is, feedback on how to work, ongoing project state, external references — gets a file in `.claude/memories/` and a one-line pointer in `MEMORY.md` beside it.
