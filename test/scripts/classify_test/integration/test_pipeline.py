...
def _stub_externals(ct, monkeypatch, tmp_path, classifier):
    ...
    monkeypatch.setattr(
        ct, "update_issue_after_commit", _noop)
    monkeypatch.setattr(
...
def _do_merge(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="Fresh")
    return existing
...
def _do_dry_run(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="DryT", dry_run=True)
    return tmp_path
...
def test_dedup_reports_duplicate(ct, tmp_path, monkeypatch, capsys):
    ...
    _run_pipeline(ct, tmp_path, test="Dup")
...
def test_non_lrm_topic_creates_named_file(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="InternalHelper")
...
def test_annex_creates_annex_file(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="GrammarRule")
...
def test_preamble_propagated_to_output(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="T")
...
def test_lrm_quotes_stripped_in_output(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="QuotedTest")
...
def test_output_reparseable(ct, tmp_path, monkeypatch):
    ...
    _run_pipeline(ct, tmp_path, test="Round")
    assert len(ct.parse_file(
        tmp_path / "test_parser_clause_06_01.cpp",
    ).all_tests) == 1
...
def test_prefix_fallback_creates_correct_file(ct, tmp_path, monkeypatch):
    ...
    _stub_externals(ct, monkeypatch, tmp_path, _make_classifier_with_prefix(
        "TimerTest", "11.4", "simulator",
    ))
    _run_pipeline(ct, tmp_path, test="TimerTest")
    assert (tmp_path / "test_simulator_clause_11_04.cpp").exists()
