...
def test_find_existing_tests_empty(tmp_path):
    ...
    assert classify_test.find_existing_tests(
...
def test_find_existing_tests_exact(tmp_path):
    ...
    result = classify_test.find_existing_tests(
        "test_parser_clause_06", tmp_path,
    )
...
def test_clause_to_filename_non_lrm_with_topic():
    ...
    assert classify_test.clause_to_filename(
...
def test_clause_to_filename_trailing_underscore():
    ...
    result = classify_test.clause_to_filename("test_parser_", "6.1")
    assert result == "test_parser_clause_06_01"


# ---- _count_file_lines -----------------------------------------------------

_count_file_lines = getattr(classify_test, "_count_file_lines")
...
_next_suffix_file = getattr(classify_test, "_next_suffix_file")
...
_rename_base_to_suffix = getattr(classify_test, "_rename_base_to_suffix")
...
def test_find_merge_target_exact(tmp_path):
    ...
    f = tmp_path / "test_parser_clause_06.cpp"
    f.write_text("")
    assert classify_test.find_merge_target(
        "test_parser_clause_06", tmp_path,
    ) == f
...
def test_find_merge_target_variant(tmp_path):
    ...
    (tmp_path / "test_parser_clause_06_a.cpp").write_text("")
    (tmp_path / "test_parser_clause_06_b.cpp").write_text("")
    result = classify_test.find_merge_target(
        "test_parser_clause_06", tmp_path,
    )
...
def test_strip_lrm_quotes_no_quote():
    ...
    assert classify_test.strip_lrm_quotes("int x = 0;") == "int x = 0;"


def test_strip_lrm_quotes_section_ref():
    """Strips quote but keeps section reference."""
    result = classify_test.strip_lrm_quotes(
...
def test_generate_file_non_lrm():
    ...
    parsed = _parsed()
    t = _tb("T", comments=[])
    content = classify_test.generate_file("non-lrm", "", parsed, [t])
...
def test_generate_file_annex_with_title():
    ...
    parsed = _parsed()
    t = _tb("T", comments=[])
    content = classify_test.generate_file("A.6", "Grammar", parsed, [t])
...
def test_generate_file_annex_no_title():
    ...
    parsed = _parsed()
    t = _tb("T", comments=[])
    content = classify_test.generate_file("A.6", "", parsed, [t])
...
def test_generate_file_regular_with_title():
    ...
    parsed = _parsed()
    t = _tb("T", comments=[])
    content = classify_test.generate_file("6.3", "Data types", parsed, [t])
...
def test_generate_file_regular_no_title():
    ...
    parsed = _parsed()
    t = _tb("T", comments=[])
    content = classify_test.generate_file("6.3", "", parsed, [t])
...
def test_generate_file_with_preamble():
    ...
    pre = classify_test.PreambleItem(lines=["struct Foo {", "};"])
    parsed = _parsed(preamble=[pre])
    t = classify_test.TestBlock(
...
def test_generate_file_with_section_preamble():
    ...
    sec = classify_test.PreambleItem(
...
def test_generate_file_preamble_stripped_empty():
    ...
    pre = classify_test.PreambleItem(
        lines=['// "All types are..."'],
    )
    parsed = _parsed(preamble=[pre])
    t = _tb("T", comments=[])
...
def test_append_tests_to_file_basic(tmp_path):
    ...
    t = _tb("New", comments=[])
    classify_test.append_tests_to_file(f, [], [t])
...
def _append_with_preamble(tmp_path, existing_content):
    ...
    pre = classify_test.PreambleItem(lines=["struct Bar {", "};"])
    t = _tb("T", comments=[])
    classify_test.append_tests_to_file(f, [pre], [t])
...
def test_append_tests_to_file_comment_only_preamble(tmp_path):
    ...
    pre = classify_test.PreambleItem(lines=["// A helper comment"])
    t = _tb("T", comments=[])
    classify_test.append_tests_to_file(f, [pre], [t])
    text = f.read_text(encoding="utf-8")
...
def test_append_splits_creates_overflow_file(tmp_path):
    ...
    classify_test.append_tests_to_file(
        f, [], [t], max_lines=50,
    )
    assert (tmp_path / "test_parser_clause_06_01_b.cpp").exists()
...
def test_append_splits_returns_new_name(tmp_path):
    ...
    f = _near_limit_file(tmp_path, "test_parser_clause_06_01_a.cpp")
    t = _make_large_test("Overflow", 20)
    new_names = classify_test.append_tests_to_file(
        f, [], [t], max_lines=50,
    )
...
_write_overflow_file = getattr(
    classify_test, "_write_overflow_file",
)
...
_batch_tests = getattr(classify_test, "_batch_tests")
...
_flush_overflow = getattr(classify_test, "_flush_overflow")
...
def test_update_cmake_removes_and_adds(monkeypatch, tmp_path):
    ...
    classify_test.update_cmake("old_test", ["new_a", "new_b"])
...
def test_update_cmake_sorted(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(classify_test, "CMAKE_PATH", cmake)
    classify_test.update_cmake("old", ["a_test"])
    text = cmake.read_text(encoding="utf-8")
