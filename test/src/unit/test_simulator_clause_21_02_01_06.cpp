...
// §21.2.1.6 (C7b): when the value is not one named by the enumeration, it prints
// according to the base type instead of an enumeration name. An uninitialized
// 4-state enum holds x, which names no member, so the base-type (decimal) form
// is used.
