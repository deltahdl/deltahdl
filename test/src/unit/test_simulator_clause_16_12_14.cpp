...
// the underlying property_expr's verdict. The true-condition case also shows the
// abort taking precedence over a contrary settled verdict (here kFail).
...
// shows the abort taking precedence over a contrary settled verdict (here kPass).
...
TEST(SvaEngineAbort, RejectIsDualOfAccept) {
  for (bool cond : {false, true}) {
    for (PropertyResult inner : {PropertyResult::kPass, PropertyResult::kFail}) {
      PropertyResult dual =
...
TEST(SvaEngineAbort, NestedOutermostOperatorTakesPrecedence) {
  ...
  EXPECT_EQ(EvalNestedAbort(/*outer_forces_true=*/true, /*outer_condition=*/true,
                            /*inner_forces_true=*/false, /*inner_condition=*/true,
                            PropertyResult::kPass),
            PropertyResult::kPass);
  // Inner reject condition true, outer accept condition not yet true: p fails.
  EXPECT_EQ(EvalNestedAbort(/*outer_forces_true=*/true, /*outer_condition=*/false,
                            /*inner_forces_true=*/false, /*inner_condition=*/true,
                            PropertyResult::kPass),
            PropertyResult::kFail);
}
...
// §16.12.14: asynchronous abort operators are evaluated at every simulation time
// step; the synchronous operators are evaluated only when the clocking event
// happens.
...
TEST(SvaEngineAbort, AbortPreservesVacuousInnerWhenConditionFalse) {
  EXPECT_EQ(EvalAbortAccept(/*abort_condition=*/false,
                            PropertyResult::kVacuousPass),
            PropertyResult::kVacuousPass);
  EXPECT_EQ(EvalAbortReject(/*abort_condition=*/false,
                            PropertyResult::kVacuousPass),
            PropertyResult::kVacuousPass);
}
...
// b fire the outer reject decides the verdict (fail); when only the inner accept
// fires it forces a pass even over a failing underlying property_expr.
TEST(SvaEngineAbort, NestedRejectOuterOverridesAcceptInner) {
  EXPECT_EQ(EvalNestedAbort(/*outer_forces_true=*/false, /*outer_condition=*/true,
                            /*inner_forces_true=*/true, /*inner_condition=*/true,
                            PropertyResult::kPass),
            PropertyResult::kFail);
  EXPECT_EQ(EvalNestedAbort(/*outer_forces_true=*/false, /*outer_condition=*/false,
                            /*inner_forces_true=*/true, /*inner_condition=*/true,
                            PropertyResult::kFail),
            PropertyResult::kPass);
}
