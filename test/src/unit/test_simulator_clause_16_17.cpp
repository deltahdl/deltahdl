...
TEST(ExpectStatement, PendingKeepsProcessBlocked) {
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kPending, /*has_else_clause=*/true),
...
TEST(ExpectStatement, SuccessTakesPassArm) {
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kPass, /*has_else_clause=*/true),
      ExpectActionKind::kRunPass);
  EXPECT_EQ(ResolveExpectAction(PropertyResult::kVacuousPass,
...
TEST(ExpectStatement, PendingBlocksRegardlessOfElseClause) {
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kPending, /*has_else_clause=*/false),
      ExpectActionKind::kBlock);
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kPending, /*has_else_clause=*/true),
      ExpectActionKind::kBlock);
...
TEST(ExpectStatement, FailureWithElseTakesElseArm) {
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kFail, /*has_else_clause=*/true),
...
TEST(ExpectStatement, FailureWithoutElseReportsViaError) {
  EXPECT_EQ(
      ResolveExpectAction(PropertyResult::kFail, /*has_else_clause=*/false),
...
TEST(ExpectStatement, FirstEvaluationOnNextClockingEvent) {
  EXPECT_FALSE(ExpectClockingEventBeginsEvaluation(
      /*activation_tick=*/10, /*clocking_event_tick=*/9));
  EXPECT_FALSE(ExpectClockingEventBeginsEvaluation(
      /*activation_tick=*/10, /*clocking_event_tick=*/10));
  EXPECT_TRUE(ExpectClockingEventBeginsEvaluation(
      /*activation_tick=*/10, /*clocking_event_tick=*/11));
}
