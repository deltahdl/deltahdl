...
// on the elaborator. Installing that order is what leaves the configuration's
// clauses something to override: without it a test could not tell a clause
// being obeyed from a map order that happened to agree. The configuration's own
// file matches no file path specification in the map, so no library the map
// declares holds the configuration and it cannot itself be what supplies a
// cell.
...
// binding off some description first states that the design held that
// description, so a design that never held it cannot pass for one whose
