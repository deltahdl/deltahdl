...
TEST(UdpBodyPreprocessor, CombinationalBodyThroughPreprocessor) {
  EXPECT_TRUE(
      ParseWithPreprocessorOk("primitive and_udp(output y, input a, b);\n"
                              "  table\n"
                              "    0 0 : 0;\n"
                              "    0 1 : 0;\n"
                              "    1 0 : 0;\n"
...
TEST(UdpBodyPreprocessor, SequentialBodyThroughPreprocessor) {
  EXPECT_TRUE(
      ParseWithPreprocessorOk("primitive dff(output reg q, input d, clk);\n"
                              "  initial q = 1'b0;\n"
                              "  table\n"
                              "    0 r : ? : 0;\n"
                              "    1 r : ? : 1;\n"
...
TEST(UdpBodyPreprocessor, MacroExpandedInitVal) {
  EXPECT_TRUE(
      ParseWithPreprocessorOk("`define INIT_VAL 1'b0\n"
...
TEST(UdpBodyPreprocessor, ConditionalCompilationInTable) {
  EXPECT_TRUE(
      ParseWithPreprocessorOk("`define FULL_TABLE\n"
...
TEST(UdpBodyPreprocessor, AllInitValFormsThroughPreprocessor) {
  for (const char* init_val : {"1'b0", "1'b1", "1'bx", "1'bX", "1'B0", "1'B1",
                               "1'Bx", "1'BX", "0", "1"}) {
    std::string src =
