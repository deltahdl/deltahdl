...
// the range satisfies the inner property_expr — even when the range's later ticks
// are not yet present, as for the unbounded `s_eventually [2:$]` form.
...
// §16.12.13: a weak `eventually` holds when the inner property_expr holds at some
// tick within the range, exactly as the strong form does.
...
// §16.12.13: a weak eventually with a witness passes even when the range's later
// ticks are not all present — the satisfying tick is reached before any missing
// tick matters, so incomplete observation of the range does not change the pass.
...
// tick (equivalent to strong(##[*0:$] property_expr)), so a witness makes it pass
// even when later clock ticks are not all present.
