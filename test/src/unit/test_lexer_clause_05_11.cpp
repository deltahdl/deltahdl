...
TEST(LexicalConventionLexing, PositionalArrayLiteralTokens) {
...
TEST(LexicalConventionLexing, NestedArrayLiteralTokens) {
...
TEST(LexicalConventionLexing, ReplicationArrayTokens) {
  auto tokens = Lex("'{3{4}}");
...
TEST(LexicalConventionLexing, IndexKeyDefaultTokens) {
...
TEST(LexicalConventionLexing, ReplicationTokens) {
  auto tokens = Lex("'{3{1}}");
  ASSERT_GE(tokens.size(), 6u);
  EXPECT_EQ(tokens[0].kind, TokenKind::kApostropheLBrace);
  EXPECT_EQ(tokens[1].kind, TokenKind::kIntLiteral);
