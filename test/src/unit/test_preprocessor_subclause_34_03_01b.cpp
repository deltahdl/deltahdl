...
// test naming the report through ReportedError, which applies to a rule the
// program reports through common/diagnostic.h; TryParseProtectArg writes to
// std::cerr and returns a bool instead, so there is no diagnostic for a case to
// name.
//
