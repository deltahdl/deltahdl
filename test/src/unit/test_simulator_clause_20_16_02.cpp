...
// personality formats (the array/plane formats of §20.16.4). The simulator's PLA
// engine (eval_systask.cpp) carries this rule: ClassifyPlaTask decodes the logic
// component of the task name and PlaReduceWord reduces each personality word with
// the corresponding function - an AND reduction for and/nand and an OR reduction
// for or/nor, with the result complemented for the nand/nor forms. These tests
// drive small arrays so that each of the four logic functions is observable from
// the output term, and exercise the nor/nand planes in the plane format and the
// asynchronous type to confirm the functions apply across types and formats.
...
// §20.16.2: all four logic planes are modeled. Driving the identical personality
// memory (both inputs taken) and inputs (one high, one low) through and/or/nand/
// nor arrays makes each function produce its own documented value: and -> 1&0=0,
// or -> 1|0=1, nand -> ~(1&0)=1, nor -> ~(1|0)=0. The four distinct results show
// that the logic component of the task name selects the modeled logic function.
...
// and both inputs high, each term is ~1 = 0, the and reduction is 0, and the nand
// complement drives the output high. The array format would instead take neither
// input (leaving the and identity 1) and read 0, so this result is specific to
// the nand function reducing in the plane format.
...
// codes are 0 (take the complemented input). Starting with both inputs low, each
// term is ~0 = 1, the or reduction is 1, and the nor complement reads 0. Raising
// both inputs makes the asynchronous array re-evaluate on its own: each term
// becomes ~1 = 0, the or reduction is 0, and the nor output rises to 1 - showing
// the nor function applied under the asynchronous type in the plane format.
