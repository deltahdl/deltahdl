...
// vpi_get_str(). They apply to every object that corresponds to something in the
// source text, with one fixed set of exceptions - object kinds that have no
// single source line or file: vpiCallback, vpiDelayTerm, vpiDelayDevice,
// vpiInterModPath, vpiIterator, vpiTimeQueue, vpiGenScopeArray, and vpiGenScope.
// (The `line directive of §22.12 may shift the reported values; that effect is
// §22.12's to define, not §37.3.3's.) These tests drive the production routines
...
// Claim (exception): vpiLineNo does not apply to the object kinds §37.3.3 lists,
// so vpi_get(vpiLineNo) on each of them is not a valid query and yields
...
TEST_F(VpiFileAndLineProperty, ExceptedKindsHaveNoLineNo) {
  ...
  for (int type : excepted) {
    VpiObject obj;
    obj.type = type;
    obj.line_no = 99;  // present in the model, but not a valid query for this kind
...
// vpi_get_str(vpiFile) yields null on each of them regardless of any file string
