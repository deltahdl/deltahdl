...
// §16.13.2: a multiclocked sequence evaluated as a property is true iff there is
// a match beginning at that point, and the verdict is always a definite true or
// false (never pending) — mirroring the singly clocked true/false result.
TEST(MulticlockedProperty, SequenceAsPropertyIsTrueIffMatch) {
  ...
  EXPECT_NE(EvalMulticlockedSequenceAsProperty(false), PropertyResult::kPending);
}

// §16.13.2: `(@(posedge clk0) sig0) and (@(posedge clk1) sig1)` is a multiclocked
// property that holds iff both differently clocked operands have a match
...
// the nearest strictly future tick of the consequent's clock. A consequent-clock
// tick coincident with the antecedent end does not qualify.
...
// §16.13.2: the overlapping implication (|->) awaits the nearest consequent-clock
// tick. When that clock ticks at the antecedent end the consequent is checked
// there immediately.
...
// overlapping implication behaves as the nonoverlapping one — it advances to the
// nearest strictly future tick and does not check immediately.
...
// §16.13.2: combination example `@(posedge clk0) s0 |=> (@(posedge clk1) s1) and
...
TEST(MulticlockedProperty, ImplicationConsequentIsMulticlockedAnd) {
  ...
  EXPECT_EQ(EvalMulticlockedAnd(/*s1=*/true, /*s2=*/true), PropertyResult::kPass);
...
// tick of its clock, which likewise admits a coincident tick and otherwise takes
...
// locate a multiclocked operand, so the search yields the no-tick sentinel under
// both the inclusive and strictly-future readings.
TEST(MulticlockedProperty, NoClockTicksYieldsSentinel) {
  ...
  EXPECT_EQ(NearestClockTickAtOrAfter(/*from=*/0, no_ticks, /*inclusive=*/true),
            kNoMulticlockTick);
  EXPECT_EQ(NearestClockTickAtOrAfter(/*from=*/0, no_ticks, /*inclusive=*/false),
            kNoMulticlockTick);
}

// §16.13.2 edge case: when the antecedent ends before the consequent clock's
// first tick, no tick coincides with the end, so both the nonoverlapping and the
// overlapping forms advance to that first tick and neither checks immediately.
...
// §16.13.2 error case: when the antecedent ends after the consequent clock's last
// tick, even the overlapping form finds no tick at or after the end, so the
// consequent cannot be located and the implication never checks immediately.
