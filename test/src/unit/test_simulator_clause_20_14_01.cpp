...
std::vector<int32_t> RandomStream(SimFixture& f, uint32_t seed, int n) {
  ...
  out.push_back(static_cast<int32_t>(EvalExpr(seeded, f.ctx, f.arena).ToUint64()));
  for (int i = 1; i < n; ++i) {
    auto* call = MkSysCall(f.arena, "$random", {});
    out.push_back(static_cast<int32_t>(EvalExpr(call, f.ctx, f.arena).ToUint64()));
  }
  return out;
}
...
// reseeds the stream and returns a new 32-bit number through the same code path,
// exercising the seed branch rather than echoing the seed.
...
// §20.14.1: a new number is produced on each call, so consecutive draws from one
// stream are not pinned to a single value.
TEST(RandomFunction, AdvancesAcrossConsecutiveCalls) {
  SimFixture f;
  std::set<int32_t> seen;
  for (int i = 0; i < 8; ++i) {
    auto* call = MkSysCall(f.arena, "$random", {});
    seen.insert(static_cast<int32_t>(EvalExpr(call, f.ctx, f.arena).ToUint64()));
  }
...
// §20.14.1: the seed argument controls the numbers $random returns, so different
// seeds generate different random streams.
