...
// §10.3.2's Example 2 writes a concatenation lvalue as the ordinary way to
// state a carry out beside a sum: `assign {carry_out, sum_out} = ina + inb +
// carry_in;`. Every element of it is a net, and §10.3.2 has "Nets ... driven by
// multiple continuous assignments or by a mixture of primitive outputs, module
// outputs, and continuous assignments" without excepting the ones a
// concatenation names. Each therefore has a driver rather than a value written
// into its storage behind resolution's back.
TEST(ContAssignStatementSim, ConcatTargetRegistersADriverOnEachNet) {
  ...
  ASSERT_NE(design, nullptr);
  LowerAndRun(design, f);
  auto* c = f.ctx.FindNet("c");
  auto* s = f.ctx.FindNet("s");
  ASSERT_NE(c, nullptr);
  ASSERT_NE(s, nullptr);
  EXPECT_EQ(c->drivers.size(), 1u);
  EXPECT_EQ(s->drivers.size(), 1u);
}

// §11.4.1 gives the leftmost element the most significant bits, so `c` takes
// bit 4 and `s` the four below it. Values chosen so that neither answer is the
// other's: 5'b10110 gives c=1 and s=4'b0110.
TEST(ContAssignStatementSim, ConcatTargetGivesEachNetItsOwnBits) {
  SimFixture f;
  auto* design = ElaborateSrc(
      "module t;\n"
      "  wire c;\n"
  ...
  ASSERT_NE(c, nullptr);
  ASSERT_NE(s, nullptr);
  EXPECT_EQ(c->resolved->value.ToUint64() & 1u, 1u);
  EXPECT_EQ(s->resolved->value.ToUint64() & 0xFu, 0x6u);
}

// The driver drives only what the concatenation named, so §6.6.5's continuous
// 0 of pull strength decides the bits of a tri0 net the target does not reach.
// Written into storage rather than resolved, the untouched half would report
// the undriven answer for the whole net.
TEST(ContAssignStatementSim, ConcatTargetLeavesTheRestOfANetToResolution) {
  ...
  ASSERT_NE(a, nullptr);
  EXPECT_EQ(a->resolved->value.ToUint64() & 0xFu, 0x3u);
  EXPECT_EQ(a->BitStrength(0).s1_hi, Strength::kStrong);
  EXPECT_EQ(a->BitStrength(2).s0_hi, Strength::kPull);
}

// A target naming a net twice takes two driver slots on it, so the two are
// combined by §28.12 rather than one overwriting the other. `{a[0], a[1]}`
// reverses the two bits it is given.
TEST(ContAssignStatementSim, ConcatTargetNamingOneNetTwiceTakesTwoSlots) {
  SimFixture f;
  auto* design = ElaborateSrc(
      "module t;\n"
      "  wire [1:0] a;\n"
      "  assign {a[0], a[1]} = 2'b10;\n"
      "endmodule\n",
      f);
  ASSERT_NE(design, nullptr);
  LowerAndRun(design, f);
  ...
  EXPECT_EQ(a->resolved->value.ToUint64() & 0x3u, 0x1u);
}
