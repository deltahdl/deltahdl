...
// §37.79 Assign statement, deassign, force, release: the object model diagram for
// the procedural continuous assignment family. The clause carries no BNF, no
// numbered Details, and no 'shall' sentences - it is the diagram alone. The
...
// vpiLhs and a value through vpiRhs; a deassign and a release each reach a target
// through vpiLhs only (they name a target but supply no value).
//
...
// relation tags, so the generic child walk in VpiHandleC - which matches by exact
// relation tag - cannot find them; they are held as designated pointers. These
...
// designated pointers, so the dispatch returns the rhs expression for vpiRhs and
// the lhs target for vpiLhs - the force's two diagram edges in one observation.
...
// target but supply no value. The rhs gate is scoped to the assign statement and
// force, so a deassign falls through to the generic walk, which matches by exact
// relation tag and reports null even though an rhs pointer happens to be set.
...
// assignment kinds, so it does not disturb the vpiLhs edge other objects draw. A
// non-family object is left to the generic traversal, which matches by exact
