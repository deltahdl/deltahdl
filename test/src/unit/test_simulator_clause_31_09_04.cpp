...
TEST(NegativeTimingCheckOption, OptionEnablesNegativeHandlingAndAppliesDelay) {
  const bool kActive = NegativeTimingCheckOptionActive(true, false);
  EXPECT_TRUE(kActive);
  EXPECT_EQ(EffectiveTimingCheckSignalDelay(7, kActive), 7);
}
...
TEST(NegativeTimingCheckOption, WithoutOptionDelayedSignalsAreCopies) {
  const bool kActive = NegativeTimingCheckOptionActive(false, false);
  EXPECT_FALSE(kActive);
  EXPECT_EQ(EffectiveTimingCheckSignalDelay(7, kActive), 0);
}
...
TEST(NegativeTimingCheckOption, AllChecksOffDelayedSignalsAreCopies) {
  const bool kActive = NegativeTimingCheckOptionActive(true, true);
  EXPECT_FALSE(kActive);
  EXPECT_EQ(EffectiveTimingCheckSignalDelay(7, kActive), 0);
}
