...
// itself. A region that came back sealed came back sealed because the first
// two really reached the third.
...
// one the region asked for, so a region left untransformed was left so for
// want of its own key rather than for want of any.
...
// The expression picking that provider's key out by name, as §34.5.12 writes
// it.
//
// It is the line a reading really does draw on, which is what makes it the
// thing to write this subclause's expression in the place of.
...
// One such region carrying `written` between the designations reaching its key
// and the design it seals.
//
