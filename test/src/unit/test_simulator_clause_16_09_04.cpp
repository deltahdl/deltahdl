...
TEST(GlobalClockingSampledValue, PastStableAndChanged) {
  ...
  EXPECT_TRUE(ChangedGclk(/*prev=*/5, /*cur=*/6));
  EXPECT_FALSE(ChangedGclk(/*prev=*/5, /*cur=*/5));
}
...
TEST(GlobalClockingSampledValue, ChangingIsComplementOfSteady) {
  EXPECT_TRUE(SteadyGclk(/*cur=*/3, /*next=*/3));
  EXPECT_FALSE(SteadyGclk(/*cur=*/3, /*next=*/4));
...
TEST(GlobalClockingSampledValue, FutureKillTimingBoundsTheAttempt) {
  EXPECT_TRUE(GclkFutureKillAffectsAttempt(/*at_or_before_last_tick=*/true));
  EXPECT_FALSE(GclkFutureKillAffectsAttempt(/*at_or_before_last_tick=*/false));
}
...
TEST(GlobalClockingSampledValue, EdgeFunctionsUseLeastSignificantBit) {
  EXPECT_TRUE(RoseGclk(/*prev=*/0b10, /*cur=*/0b11));   // LSB 0 -> 1
  EXPECT_FALSE(RoseGclk(/*prev=*/0b11, /*cur=*/0b10));  // LSB 1 -> 0
  EXPECT_TRUE(FellGclk(/*prev=*/0b01, /*cur=*/0b10));   // LSB 1 -> 0
  EXPECT_TRUE(RisingGclk(/*cur=*/0b100, /*next=*/0b101));
  EXPECT_TRUE(FallingGclk(/*cur=*/0b011, /*next=*/0b010));
}
...
TEST(GlobalClockingSampledValue, StableComparesWholeValue) {
  EXPECT_FALSE(StableGclk(/*prev=*/0b10, /*cur=*/0b00));  // upper bit changed
  EXPECT_TRUE(ChangedGclk(/*prev=*/0b10, /*cur=*/0b00));
}
