...
TEST(LetDeclParsing, DeclNoArgsParse) {
  auto r = Parse(
      "module t;\n"
...
TEST(LetDeclParsing, DeclWithArgsParse) {
  auto r = Parse(
      "module t;\n"
      "  let op(x, y, z) = |((x | y) & z);\n"
      "endmodule\n");
...
TEST(LetDeclParsing, DeclWithDefaultsParse) {
  auto r = Parse(
      "module t;\n"
...
TEST(LetDeclParsing, DeclTypedArgsParse) {
  auto r = Parse(
      "module t;\n"
...
TEST(LetDeclParsing, DeclUntypedArg) {
  auto r = Parse(
      "module t;\n"
...
TEST(LetDeclParsing, DeclEmptyParens) {
  auto r = Parse(
      "module t;\n"
      "  let empty_let() = 42;\n"
      "endmodule\n");
...
TEST(LetDeclParsing, BasicLetDeclWithArg) {
  auto r = Parse(
      "module m;\n"
      "  let my_let(a) = a + 1;\n"
      "endmodule\n");
...
// --- let_expression (instantiation) ---

TEST(LetDeclParsing, LetCallBasic) {
  auto r = Parse(
      "module t;\n"
      "  let op(x, y) = x + y;\n"
      "  initial begin\n"
...
TEST(LetDeclParsing, LetCallNamedArgs) {
  auto r = Parse(
      "module t;\n"
...
TEST(LetDeclParsing, LetCallWithDefaults) {
  auto r = Parse(
      "module t;\n"
      "  let at_least_two(sig, rst = 1'b0) = rst || sig;\n"
      "  initial begin\n"
...
TEST(LetDeclParsing, DeclInPackage) {
  auto r = Parse(
      "package pkg;\n"
      "  let my_op(x, y) = x & y;\n"
      "endpackage\n");
...
TEST(LetDeclParsing, LetDeclInBlock) {
  EXPECT_TRUE(
      ParseOk("module m;\n"
              "  initial begin\n"
              "    let my_add(x, y) = x + y;\n"
...
TEST(LetDeclParsing, LetDeclWithTernaryBody) {
  EXPECT_TRUE(
      ParseOk("module m;\n"
              "  let max(a, b) = (a > b) ? a : b;\n"
              "endmodule\n"));
...
TEST(LetDeclParsing, LetCallInContinuousAssign) {
  EXPECT_TRUE(
      ParseOk("module m;\n"
              "  let add(a, b) = a + b;\n"
              "  logic [7:0] w;\n"
              "  assign w = add(8'd3, 8'd4);\n"
              "endmodule\n"));
}
