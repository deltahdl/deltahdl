...
TEST(QInitialize, LifoTypeIsAccepted) {
  SimFixture f;
  EXPECT_EQ(QInitialize(f, 1, /*q_type=*/2, /*max_length=*/4), 0u);
}
...
TEST(QInitialize, TypeOutsideTableCreatesNothing) {
  SimFixture f;
  EXPECT_NE(QInitialize(f, 1, /*q_type=*/3, /*max_length=*/4), 0u);
...
TEST(QInitialize, TypeBelowTableCreatesNothing) {
  SimFixture f;
  EXPECT_NE(QInitialize(f, 1, /*q_type=*/0, /*max_length=*/4), 0u);
  EXPECT_NE(QueueDefined(f, 1), 0u);  // creation did not register a queue
}
...
TEST(QInitialize, MaxLengthBoundsTheEntryCount) {
  ...
  EXPECT_EQ(QAdd(f, 1), 0u);  // entry 1 of 2
  EXPECT_EQ(QAdd(f, 1), 0u);  // entry 2 of 2 fills the queue
  EXPECT_NE(QAdd(f, 1), 0u);  // capacity reached, cannot add a third
}
...
TEST(QInitialize, QidMustBeUnique) {
  ...
  EXPECT_NE(QInitialize(f, 5, 1, 4), 0u);
}
...
TEST(QInitialize, DistinctQidsAreIndependentQueues) {
  SimFixture f;
  EXPECT_EQ(QInitialize(f, 1, 1, 4), 0u);
  EXPECT_EQ(QInitialize(f, 2, 1, 4), 0u);
  EXPECT_EQ(QAdd(f, 1), 0u);           // populate queue 1 only
  EXPECT_EQ(QRemoveStatus(f, 1), 0u);  // queue 1 yields its entry
  EXPECT_NE(QRemoveStatus(f, 2), 0u);  // queue 2 is still empty
}
