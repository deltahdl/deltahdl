...
// §6.18: a typedef name stands for the type it was declared with, so a function
// returning `nib` returns the four bits `nib` is. The simulator has no typedef
// table -- EvalTypeWidth answers 0 for a named type and the caller sizing the
// return variable turned that 0 into 32 -- so the name was resolved nowhere and
// the caller read 255.
//
// The typedef is four bits wide rather than thirty-two, because a typedef of
// exactly the fallback width makes the resolved answer and the unresolved one
// the same number and cannot fail.
TEST(FunctionReturnSim, ReturnTypeWrittenAsATypedefTakesThatTypesWidth) {
  uint64_t r = RunAndGet(
      "module t;\n"
      "  typedef bit [3:0] nib;\n"
      "  int r;\n"
...
// The same rule where the typedef is wider than the fallback rather than
// narrower, which the case above leaves unstated: a fix that clamped every
// unresolved name to something small would pass it and lose these bits.
// 40'h12_3456_789A is above 2^32, so a return sized at the fallback's
// thirty-two would drop the 0x12 the assertion reads back.
TEST(FunctionReturnSim, ReturnTypeWrittenAsAWideTypedefKeepsItsHighBits) {
  uint64_t r = RunAndGet(
      "module t;\n"
      "  typedef bit [39:0] wide;\n"
      "  longint r;\n"
      "  function wide f();\n"
      "    return 40'h12_3456_789A;\n"
      "  endfunction\n"
      "  initial begin\n"
      "    r = f();\n"
      "  end\n"
