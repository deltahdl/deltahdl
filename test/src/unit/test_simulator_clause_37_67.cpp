...
// §37.67 Waits: the object model diagram groups three wait statements - a wait, an
// ordered wait, and a wait fork - under the abstract "waits" label. It draws a
// vpiCondition edge from the wait and ordered wait to a controlling condition that
// is either an expression or a sequence instance, an unlabeled edge from the
// grouping to a body statement (the vpiStmt relation), and a vpiElseStmt edge to an
// else action statement. The clause carries no numbered Details, no BNF, and no
// 'shall' sentences. These tests observe the production code that serves the
// diagram's relations: the vpiCondition edge through the dedicated helper
// VpiWaitConditionExpr, and the body and else edges through the generic traversal.
...
// exercises the sequence-instance branch the diagram draws alongside the
...
// returns the first expression-or-sequence child.
TEST_F(Waits, ConditionFoundAfterPrecedingBodyChild) {
  VpiObject body;
  body.type = vpiStmt;  // neither an expression nor a sequence inst, listed first
...
// vpiCondition edge: a null handle and a wait with no condition child both report
// no condition. A wait fork draws no condition edge, so it stands in for the
// no-condition case here.
...
// draws and rejects unrelated statement kinds (here a while loop, which belongs to
// the separate §37.66 grouping).
...
// Else edge (the diagram's vpiElseStmt arrow to a statement): a wait fork reaches
// its else action statement through the generic vpiElseStmt traversal, since the
// else child carries the relation tag as its own type.
