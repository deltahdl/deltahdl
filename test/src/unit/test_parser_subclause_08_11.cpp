...
// 187 of ~/LRM.pdf), and a name reached through it takes the same postfix
// chain as any other name: A.8.4's method_call and A.8.6's select run on,
// so `this.m[k].kill()` calls a method of a selected element. The parser read
// one call and one select after the `this.` chain and stopped, so the `.`
