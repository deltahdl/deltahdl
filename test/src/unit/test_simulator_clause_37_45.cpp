...
// §37.45 Delay terminals: the object model diagram for a delay device and its two
// delay terminals. A delay device reaches its input delay term through vpiInTerm
// and its output delay term through vpiOutTerm; both terms are delay term
// objects. The delay device and each delay term carry the vpiDelayType integer
// property, and a delay device is enclosed by a module. The subclause's two
// numbered Details fix the run-time ordering of value changes on the terminals
// relative to the device's delay (the input term changes before the delay, the
// output term not until after it); those are event-scheduling semantics with no
// VPI query to observe, so they carry no production hook here. The cross-edges to
// net drivers / net loads (vpiDriver / vpiLoad) and the terminal value
// (vpi_get_value) are represented by other subclauses. These tests observe the
// production code that applies the delay-device relations and property through
// the public vpi_handle / vpi_get dispatch.
...
// vpiInTerm / vpiOutTerm (scope): the terminal relations are specific to a delay
// device. On an object that is not a delay device, asking for them does not
// reach a stray delay term, even when one is present as a child.
...
// vpiDelayType: a delay device and a delay term both report their delay-type code
// through vpi_get(vpiDelayType); an object that is neither reports zero.
...
// module -- delay device: a delay device is enclosed by a module, reached through
// the generic parent walk. A delay device with no module parent reports none.
