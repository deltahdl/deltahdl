...
struct Design {
  ...
  bool Build(const std::string& src) {
    ...
    // §32.9: a $sdf_annotate call reaches the design's specify data through
    // the running context, so binding the manager here is what lets the calls
    // the source writes annotate this design.
