...
TEST(Past, GatingExpressionCountsOnlyActiveTicks) {
  ...
  const Word word{A({"en"}), A({}), A({"en"}), A({})};
  // One active tick back from j=3 is the active letter at i=2.
  EXPECT_EQ(PastSourceIndices(word, /*j=*/3, /*n=*/1, en, One()), Indices{2});
  // Two active ticks back from j=3 is the active letter at i=0.
  EXPECT_EQ(PastSourceIndices(word, /*j=*/3, /*n=*/2, en, One()), Indices{0});
}
...
TEST(Past, ClockGatesTicks) {
  ...
  const Word word{A({}), A({"clk"}), A({}), A({"clk"})};
  // j must be in range and a tick must lie strictly before it: one tick back
  // from j=3 is i=1.
  EXPECT_EQ(PastSourceIndices(word, /*j=*/3, /*n=*/1, One(), clk), Indices{1});
}
...
TEST(Past, ClockAndGateBothRequiredForATick) {
  ...
  const Word word{A({"clk", "en"}), A({"clk"}), A({"clk", "en"})};
  // One tick back from j=2 skips the clk-only letter 1 and lands on i=0.
  EXPECT_EQ(PastSourceIndices(word, /*j=*/2, /*n=*/1, en, clk), Indices{0});
}
...
TEST(Past, FallsBackToInitialValueWhenTooFewTicks) {
  ...
  const Word word{A({"en"}), A({}), A({})};
  EXPECT_TRUE(PastSourceIndices(word, /*j=*/2, /*n=*/2, en, One()).empty());
  EXPECT_TRUE(PastSamplesInitialValue(word, /*j=*/2, /*n=*/2, en, One()));
  // The reachable single-tick case does sample, so it is not the initial value.
  EXPECT_FALSE(PastSamplesInitialValue(word, /*j=*/2, /*n=*/1, en, One()));
}
...
TEST(Past, TakesInitialValueAtTheFirstLetter) {
  const Word word{A({"en"}), A({"en"})};
  auto en = BoolAtom("en");
  EXPECT_TRUE(PastSourceIndices(word, /*j=*/0, /*n=*/1, en, One()).empty());
  EXPECT_TRUE(PastSamplesInitialValue(word, /*j=*/0, /*n=*/1, en, One()));
}
...
TEST(Past, RejectsOutOfRangePoint) {
  const Word word{A({}), A({})};
  EXPECT_TRUE(PastSourceIndices(word, /*j=*/2, /*n=*/1, One(), One()).empty());
  EXPECT_FALSE(PastSamplesInitialValue(word, /*j=*/2, /*n=*/1, One(), One()));
}
...
TEST(Past, RejectsZeroTickCount) {
  const Word word{A({}), A({})};
  EXPECT_TRUE(PastSourceIndices(word, /*j=*/1, /*n=*/0, One(), One()).empty());
  EXPECT_FALSE(PastSamplesInitialValue(word, /*j=*/1, /*n=*/0, One(), One()));
}
...
TEST(PastGclk, SamplesThePrecedingLetter) {
  const Word word{A({}), A({}), A({})};
  EXPECT_EQ(PastGclkSourceIndex(word, /*j=*/2), std::optional<std::size_t>{1});
  EXPECT_EQ(PastGclkSourceIndex(word, /*j=*/1), std::optional<std::size_t>{0});
}
