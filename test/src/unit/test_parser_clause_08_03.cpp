...
TEST(ClassSyntaxParsing, StaticPropertyDeclaration) {
  auto r = Parse(
      "class Packet;\n"
...
TEST(ClassSyntaxParsing, StaticPropertyWithInitializer) {
  auto r = Parse(
      "class Packet;\n"
      "  static int count = 0;\n"
      "endclass\n");
...
TEST(ClassSyntaxParsing, MixedStaticAndInstanceProperties) {
  auto r = Parse(
      "class C;\n"
...
// === class_constructor_declaration: super.new ===

TEST(ClassSyntaxParsing, ConstructorSuperNew) {
  auto r = Parse(
      "class Base;\n"
      "  function new();\n"
      "  endfunction\n"
...
TEST(ClassSyntaxParsing, SuperNewExpression) {
  auto r = Parse(
      "class Base;\n"
      "  function new(int x);\n"
      "  endfunction\n"
...
TEST(ClassSyntaxParsing, ConstructorChainingDefault) {
  auto r = Parse(
      "class Base;\n"
...
TEST(ClassSyntaxParsing, SuperNewWithMultipleArgs) {
  EXPECT_TRUE(
      ParseOk("class Base;\n"
...
TEST(ClassSyntaxParsing, ConstructorSuperNewDefault) {
  ParseOk(
      "class Base;\n"
      "  function new();\n"
      "  endfunction\n"
