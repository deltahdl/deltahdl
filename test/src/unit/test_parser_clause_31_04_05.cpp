...
TEST(SystemTimingCheckParsing, SystemTimingCheckPeriod) {
  auto r = Parse(
      "module m;\n"
      "specify\n"
...
TEST(TimingCheckCommandParsing, PeriodTimingCheck) {
  auto r = Parse(
      "module m;\n"
      "specify\n"
...
TEST(GateLevelModelingParsing, TimingCheckPeriod) {
...
TEST(PeriodTimingCheck, PeriodBasic) {
  auto r = Parse(
      "module m;\n"
      "specify\n"
      "  $period(posedge clk, 50);\n"
...
TEST(PeriodWithNotifier, PeriodWithNotifier) {
  auto r = Parse(
      "module m;\n"
      "specify\n"
      "  $period(posedge clk, 50, ntfr);\n"
