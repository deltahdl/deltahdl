...
// port's $var declaration uses (§21.7.4.2). These tests drive the same VcdWriter
// that emits the file (the output stage), with the extended port form selected by
// SetExtendedPortNodes().
...
//   - both strength components are present and encoded as strength-value digits:
...
TEST_F(ExtendedVcdValueChangeSim, ScalarPortValueChangesUseExtendedForm) {
  ...
  // Each driven port: p, the binary state, strong/strong (6 6), then its integer
  // identifier code.
...
// §21.7.4.3: the extended format has no mechanism to dump part of a vector, so a
// bus port's value change carries the whole vector — one port_value state
...
// code preceded by <, the same integer used in its $var declaration (§21.7.4.2).
// Because it is an integer rather than a single printable character (as in the
// 4-state format), it is not limited to one digit: a design with more than ten
// ports yields multi-digit codes. This edge case registers eleven ports so the
// last is assigned the two-digit code 10, and confirms the value change carries
// that integer verbatim rather than truncating or remapping it to one character.
