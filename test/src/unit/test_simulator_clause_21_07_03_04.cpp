...
// The optional trailing filename names a $dumpports output but is dispatched the
// same way by this single-file writer, so with or without it the limit covers
// the one dump: once the file grows past the budget the dumper stops and a
// comment marking the limit is inserted, while value changes written below the
// limit are retained.
TEST_F(DumpportslimitSysTask, LimitStopsPortDumpAndInsertsComment) {
  ...
  auto* data = MakeVar(f, "data", 8, 0x00);
  {
    ...
    // Apply a modest byte budget through the production $dumpportslimit path; the
