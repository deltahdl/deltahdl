...
TEST(ConfigParsing, FullDeclarationWithAllRuleTypes) {
...
TEST(ConfigParsing, DeclarationAtCompilationUnitScope) {
  auto r = ParseWithPreprocessor(
...
TEST(ConfigParsing, MacroInConfigBody) {
  auto r = ParseWithPreprocessor(
      "`define DEFAULT_LIB work\n"
      "config cfg;\n"
      "  design `DEFAULT_LIB.top;\n"
...
TEST(ConfigParsing, IfdefSelectsRule) {
  auto r = ParseWithPreprocessor(
      "`define USE_GATE_LIB\n"
      "config cfg;\n"
      "  design top;\n"
      "`ifdef USE_GATE_LIB\n"
      "  default liblist gate_lib;\n"
      "`else\n"
  ...
  ASSERT_EQ(r.cu->configs[0]->rules.size(), 1u);
  ASSERT_EQ(r.cu->configs[0]->rules[0]->liblist.size(), 1u);
  EXPECT_EQ(r.cu->configs[0]->rules[0]->liblist[0], "gate_lib");
}

TEST(ConfigParsing, IfndefSelectsElseBranch) {
  auto r = ParseWithPreprocessor(
      "config cfg;\n"
      "  design top;\n"
      "`ifndef UNDEFINED_MACRO\n"
      "  default liblist fallback_lib;\n"
      "`endif\n"
      "endconfig\n");
  ASSERT_NE(r.cu, nullptr);
  EXPECT_FALSE(r.has_errors);
