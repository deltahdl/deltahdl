...
TEST(EvalSteps, ContextWidthPropagates) {
  ...
  MakeVar(f, "a", 4, 0xF);
  MakeVar(f, "b", 4, 1);
  auto* expr = MakeBinary(f.arena, TokenKind::kPlus,
...
TEST(EvalSteps, ShiftAmountSelfDetermined) {
  ...
  MakeVar(f, "v", 8, 0x01);
  MakeVar(f, "sh", 4, 3);
  auto* expr = MakeBinary(f.arena, TokenKind::kLtLt,
                          MakeId(f.arena, "v"), MakeId(f.arena, "sh"));
...
TEST(EvalSteps, SignedComparisonExtends) {
  ...
  auto* expr = MakeBinary(f.arena, TokenKind::kLt,
                          MakeId(f.arena, "s4"), MakeId(f.arena, "s8"));
  auto result = EvalExpr(expr, f.ctx, f.arena);
  // -1 < 1 → true
...
TEST(EvalSteps, RelationalOperandsMutuallyDetermined) {
  ...
  MakeVar(f, "r4", 4, 0xF);
  MakeVar(f, "r8", 8, 0x0F);
  auto* expr = MakeBinary(f.arena, TokenKind::kEqEq,
                          MakeId(f.arena, "r4"), MakeId(f.arena, "r8"));
  auto result = EvalExpr(expr, f.ctx, f.arena);
  // 4'hF zero-extended to 8 bits = 8'h0F, equals 8'h0F → 1.
  EXPECT_EQ(result.ToUint64(), 1u);
}

// §11.8.2: Ternary condition is self-determined; branches are context-determined.
